...
**[MOVING.md](MOVING.md)** sets it up on a new machine, in order, from one
command.

**[INSTRUCTIONS.md](INSTRUCTIONS.md)** covers daily use, backups, indexing, and
...
Two databases are needed and neither is in this repo: `shabads.db`, the
library itself, which is personal and irreplaceable; and `banidb.db`, the
Gurbani corpus, 91 MB and regenerable from BaniDB.
...
converted to vectors and never shown to a reader. That output lives in a layer
